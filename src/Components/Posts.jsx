import { Card, CardContent, CardHeader, Fab, Grid, CardActions, Button, useMediaQuery, Dialog, Typography, DialogContent, Skeleton, Stack,  TextField, MenuItem, FormControl, InputLabel, Select, Box } from "@mui/material";
...
export const Posts = ({ posts, userId,setPosts }) => {
    ...
    return <>
        ...
        <Dialog fullScreen={fullScreen} open={showPost} onClose={() => setShowPost(false)}  >
            <DialogContent>
                <Grid container direction='column' spacing={2} >
                    ...
                    <Grid item ><Typography variant='body2' color='primary'>comment</Typography></Grid>
        ...
        <Fab color="primary" aria-label="add" style={{ position: 'fixed', bottom: 10, right: 10 }} onClick={() => setShowAddPost(true)}>
            <AddIcon />
        </Fab>

        <Dialog fullScreen={fullScreen} open={showAddPost} onClose={() => setShowAddPost(false)}  >
            <DialogContent>
                <Grid container direction='column' spacing={2} >
                    <Grid item >
                    ...
                    <Grid item><TextField label='title' value={newPost.title} onChange={(e) => setNewPost({ ...newPost, title: e.target.value })} /></Grid>
                    <Grid item><TextField label='body' value={newPost.body} onChange={(e) => setNewPost({ ...newPost, body: e.target.value })} /></Grid>
                    ...
                    {!userId && <Grid item>
                        <Box sx={{ minWidth: 120 }}>
                            <FormControl fullWidth>
                                ...
                                >
                                    {users.map(option => <MenuItem value={option.id}>{option.email}</MenuItem>)}
